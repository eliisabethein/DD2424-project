print("Starting YELP VAE application")
import json
import nltk
import torch
...
if not os.path.exists("results-vae"):
    os.mkdir("results-vae")
    print("Directory 'results-vae' created ")
else:
    print("Directory 'results-vae' already exists")
...
total_epoch_losses, total_kl_losses, total_mi, val_total_epoch_losses, val_total_kl_losses, val_total_mi = util.train(vae, yelp_train_inputs, yelp_train_targets,
      yelp_val_inputs, yelp_val_targets, epochs, vocabulary_size,
      hidden_size, latent_size, max_sentence_length, device, yelp_train_lengths, yelp_test_lengths, learning_rate=learning_rate,
      synthetic=False, step=step, tracked_inputs=None, tracked_targets=None, verbose=True)

pickle.dump(total_epoch_losses, open("results-vae/yelp_total_epoch_losses_vae.pkl", "wb"))
pickle.dump(total_kl_losses, open("results-vae/yelp_total_kl_losses_vae.pkl", "wb"))
pickle.dump(total_mi, open("results-vae/yelp_total_mi_vae.pkl", "wb"))

pickle.dump(val_total_epoch_losses, open("results-vae/yelp_val_total_epoch_losses_vae.pkl", "wb"))
pickle.dump(val_total_kl_losses, open("results-vae/yelp_val_total_kl_losses_vae.pkl", "wb"))
pickle.dump(val_total_mi, open("results-vae/yelp_val_total_mi_vae.pkl", "wb"))

torch.save(vae.state_dict(), "results-vae/yelp-vae.pwf")
