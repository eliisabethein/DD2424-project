print("starting application: yelp data, basic vae")
import json
import nltk
import torch
...
if not os.path.exists("results-vae-basic-32"):
    os.mkdir("results-vae-basic-32")
    print("Directory 'results-vae-basic-32' created ")
else:
    print("Directory 'results-vae-basic-32' already exists")
...
total_epoch_losses, total_kl_losses, total_mi, val_total_epoch_losses, val_total_kl_losses, val_total_mi = util.train(
        ...
        yelp_train_lengths, yelp_test_lengths, learning_rate=learning_rate,
        synthetic=False, step=step, tracked_inputs=None, tracked_targets=None, verbose=True)

pickle.dump(total_epoch_losses, open("results-vae-basic-32/yelp_total_epoch_losses_vae_basic_32.pkl", "wb"))
pickle.dump(total_kl_losses, open("results-vae-basic-32/yelp_total_kl_losses_vae_basic_32.pkl", "wb"))
pickle.dump(total_mi, open("results-vae-basic-32/yelp_total_mi_vae_basic_32.pkl", "wb"))

pickle.dump(val_total_epoch_losses, open("results-vae-basic-32/yelp_val_total_epoch_losses_vae_basic_32.pkl", "wb"))
pickle.dump(val_total_kl_losses, open("results-vae-basic-32/yelp_val_total_kl_losses_vae_basic_32.pkl", "wb"))
pickle.dump(val_total_mi, open("results-vae-basic-32/yelp_val_total_mi_vae_basic_32.pkl", "wb"))

torch.save(vae.state_dict(), "results-vae-basic-32/yelp-vae-basic-32.pwf")
